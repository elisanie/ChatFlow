            ...
            new Thread(() -> {
                ...
                try {
                    ...
                    while (true) {
                        ...
                        boolean success = client.sendAndWait(json);
                        long end = System.nanoTime();

                        if (success) {
                            metrics.recordSuccess();
                            metrics.recordLatency(new LatencyRecord(start, end, msg.getMessageType(), roomId, 200));
                        } else {
                            metrics.recordFail();
                            metrics.recordLatency(new LatencyRecord(start, end, msg.getMessageType(), roomId, 408));
                        }
                    }
                } catch (Exception e) {
